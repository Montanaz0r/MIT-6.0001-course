...
def test_is_valid_word(word_list = load_words()):
    """
...
def test_wildcard(word_list = load_words()):
    """
    Unit test for is_valid_word
    """
    failure = False
...
def test_calchand():
    ...
    if calculate_handlen(hand) != 8:
        print("FAILURE: test_calchand()")
        print(f'Expected 8, but got {calculate_handlen(hand)}.')
        failure = True

    # test 2
    hand = {'c': 1, 'o': 0, '*': 1, 'w': 1, 's':1, 'z':1, 'y': 2}

    if calculate_handlen(hand) != 7:
        print("FAILURE: test_calchand()")
        print(f'Expected 7, but got {calculate_handlen(hand)}.')
        failure = True

    # test 3
    hand = {'c': 0, 'o': 0, '*': 0, 'w': 0, 's': 0, 'z': 0, 'y': 0}

    if calculate_handlen(hand) != 0:
        print("FAILURE: test_calchand()")
        print(f'Expected 0, but got {calculate_handlen(hand)}.')
        failure = True

    if not failure:
        print('SUCCESS: test_calchand()')
...
def test_substitute():
    ...
    if letter in substitute_hand(hand, letter).keys():
        print('FAILURE: test substitute_hand()')
        print(f'{letter} was not expected to be in new hand!')
        failure = True
    ...
    if substitute_hand(hand, letter) != hand:
        print('FAILURE: test substitute_hand()')
        print('Hand was not expected to be the same one as a new hand!')
        failure = True
